...
<section
	...
	class="container w-full mx-auto px-2 md:px-4 lg:px-10"
>
	<Accordion.Root class="w-full max-w-screen" type="multiple">
		{#each list || [] as item, i}
			<Accordion.Item value={`acc-item-${i}`} class="group border-0 px-1.5 w-full ">
				<div style:gap={blockData.style?.gap}>
					<Accordion.Trigger
						class="flex font-serif font-semibold font-2xl w-full items-center justify-between select-none [&[data-state=open]svg]:rotate-180 rounded-none py-0"
					>
						{#snippet child({ props })}
							<div
				...
				>
					<div style:padding={blockData.dropStyle?.padding}>
						<div
